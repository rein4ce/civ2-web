...
var Map = function()
{
	...
	this.init = function(sizeX, sizeY)
	{
		...
		map.width = sizeX;
		map.height = sizeY;
		map.actualWidth = map.width * TileWidth;
		map.actualHeight = (this.height + 1) * TileHeight / 2;
		map.view = new Rect(0,0,Width,Height);
		map.tileset = [
			Images["img/terrain1.png"],
			Images["img/terrain2.png"]
		];
		$mapCanvas.attr("width", map.actualWidth);
	...
	this.generateRandom = function()
	{
		...
		{
			map.tiles.push([]);
			for (var x=0; x<map.width-(y%2==0?0:1); x++)
			{
				...
				map.tiles[y].push(tile);// new Tile((Math.random() * TileTypesList.length)|0) );
			}
		}
		...
		for (var y=0; y<map.tiles.length; y++)
			...
			{
				if (map.tiles[y][x].type != TileTypes.Ocean.id) continue;
	 ...
	 * @param numy
	 */
	this.drawTiles = function(x,y,numx,numy, options)
	{
	 ...
	 * @param [variant]
	 */
	this.getTilesetTileCoord = function(tileId, variant)
	{
		variant = variant || 0;
	 ...
	 * @param [options]
	 */
	this.drawTile = function(x,y,drawNeighbours,top, options)
	{
...
/*
	...
	this.initEvents = function()
	{
		...
		$container.bind("mousedown.map", function(e)
		{
			...
			if (e.which == 1)
			{
				map.tiles[map.hover.y][map.hover.x].type = map.editor.tile;
				/*if (map.getTile(map.hover.x, map.hover.y).units.length == 0)
				{
					//new Unit((Math.random()*UnitTypes.length)|0, map.hover.x, map.hover.y);
				}*/
				map.drawTile(map.hover.x, map.hover.y, true);
			}
		});
		...
		$("#map_view").unbind("mousemove").mousemove(function(e)
		{
			...
			{
				map.cursor.x = mapMouseX = e.originalEvent.layerX;
				map.cursor.y = mapMouseY = e.originalEvent.layerY;
			}
			else
			{
				map.cursor.x = mapMouseX = e.offsetX;
				map.cursor.y = mapMouseY = e.offsetY;
			}
			...
			var tile = map.screenToTile(mapMouseX, mapMouseY);
			if (map.checkTile(tile.x, tile.y))
			{
				map.hover = tile;
			...
			if (map.dragging)
			{
				var newX = e.pageX - map.dragX;
				var newY = e.pageY - map.dragY;
				if (newX > 0 || newX < $container.width() - map.actualWidth) map.dragX = e.pageX - map.x;
				if (newY > 0 || newY < $container.height() - map.actualHeight) map.dragY = e.pageY - map.y;
				var newX = Math.min(0, Math.max(newX, $container.width() - map.actualWidth));
				var newY = Math.min(0, Math.max(newY, $container.height() - map.actualHeight));
